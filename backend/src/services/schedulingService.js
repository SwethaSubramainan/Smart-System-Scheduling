...
exports.generateSchedule = async () => {
    ...
    try {
        ...
        const [availableMachines] = await connection.query(
            'SELECT * FROM machines WHERE status = "Idle"'
        );
        const [availableWorkers] = await connection.query(
        ...
        for (const job of jobs) {
            ...
            if (machineIndex !== -1 && workerIndex !== -1) {
                ...
                await connection.query(
                    'INSERT INTO schedules (job_id, machine_id, worker_id, start_time, end_time) VALUES (?, ?, ?, ?, ?)',
                    [job.id, machine.id, worker.id, startTime, endTime]
                );

                // Update Job Status
                await connection.query(
                    'UPDATE jobs SET status = "In Progress" WHERE id = ?',
                    [job.id]
                );
...
exports.handleMachineBreakdown = async (machineId) => {
    ...
    try {
        ...
        await connection.query('UPDATE machines SET status = "Maintenance" WHERE id = ?', [machineId]);
      ...
      WHERE s.machine_id = ? AND j.status = "In Progress"
    `, [machineId]);

        // 3. For each affected schedule: return job to queue, free worker
        for (const schedule of schedules) {
            // Revert Job to 'Pending' (or 'Delayed')
            await connection.query('UPDATE jobs SET status = "Delayed" WHERE id = ?', [schedule.job_id]);

            // Free worker
            await connection.query('UPDATE workers SET status = "Available" WHERE id = ?', [schedule.worker_id]);

            // Remove or mark schedule as interrupted (for simplicity we will delete the broken schedule)
            await connection.query('DELETE FROM schedules WHERE id = ?', [schedule.schedule_id]);
        }
...
exports.getDashboardKPIs = async () => {
    try {
        ...
        const [[{ total_machines }]] = await db.query('SELECT COUNT(*) as total_machines FROM machines');
        const [[{ running_machines }]] = await db.query('SELECT COUNT(*) as running_machines FROM machines WHERE status = "Running"');
        const machineUtilization = total_machines > 0 ? Math.round((running_machines / total_machines) * 100) : 0;

        // 2. Worker Utilization
        const [[{ total_workers }]] = await db.query('SELECT COUNT(*) as total_workers FROM workers');
        const [[{ busy_workers }]] = await db.query('SELECT COUNT(*) as busy_workers FROM workers WHERE status = "Busy"');
        const workerUtilization = total_workers > 0 ? Math.round((busy_workers / total_workers) * 100) : 0;

        // 3. On-time delivery rate (Simplified calculation: completed / (completed + delayed) or just mock based on priority)
        const [[{ completed_jobs }]] = await db.query('SELECT COUNT(*) as completed_jobs FROM jobs WHERE status = "Completed"');
        const [[{ delayed_count }]] = await db.query('SELECT COUNT(*) as delayed_count FROM jobs WHERE status = "Delayed"');
